...
token <- create_token(
  ...
  consumer_key = "7CPcgdG3BEw8p1MOtCMCCIMJx",
  consumer_secret = "xX4kvoLxI45IPpq2SxKhwhwtN8nbUwGM5D3w4Zf0S4uCjogK0v",
  access_token = "1016477615849234432-NPelRwghHLlUKOcm0LhpXzPunAWVRK",
  access_secret = "jziGHz4BKpIjoahSRLrMv4rWmfgIqrz7aaMoyLJJdN3AN")
...
1. Add a theme_minimal() line, which takes away the grey background.  
...
The hour() function from the lubridate package can extracts the hour of the day from a date variable such as created_at. Set pageLength to 24 in datatable - it doesn't look right in RStudio, but it will when published on a web page.
...
This tells us that the time zone the tweet is in is UTC, Coordinated Universal Time, which is the time in London. That's not the time zone Pelosi was in, it's the time zone all tweets all over the world are given. No matter what time it is in your local time zone, the time provided by rtweet is in UTC time.
